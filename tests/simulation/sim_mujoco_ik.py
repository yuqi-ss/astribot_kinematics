...
def main() -> None:
    ...
    section("MuJoCo closed-loop IK validation (two-step: torso -> arms)")
    ...
    for i, idx in enumerate(frame_indices):
        target_torso = traj.gt[CHAIN_TORSO][idx]
        target_left = traj.gt[CHAIN_ARM_LEFT][idx]
        target_right = traj.gt[CHAIN_ARM_RIGHT][idx]

        t0 = time.perf_counter()
        q1, ok1, info1 = ik_torso.solve(
            {CHAIN_TORSO: target_torso}, q_init=q_init,
            max_iters=args.max_iters, tol_pos=args.tol_pos,
            tol_rot=args.tol_rot, damping=args.damping,
        )
        q_sol, ok2, info2 = ik_arms.solve(
            {CHAIN_ARM_LEFT: target_left, CHAIN_ARM_RIGHT: target_right}, q_init=q1,
            max_iters=args.max_iters, tol_pos=args.tol_pos,
            tol_rot=args.tol_rot, damping=args.damping,
        )
        result.per_frame_time_s[i] = time.perf_counter() - t0
        result.converged[i] = bool(ok1 and ok2)
        result.iterations[i] = int(info1.get("iterations", 0)) + int(info2.get("iterations", 0))
        ...
        for key in EEF_KEYS:
            ...
            result.pos_err_mm[key][i] = position_error_mm(mj_pose_world, targets_by_key[key])
            result.rot_err_deg[key][i] = quat_angle_deg(mj_pose_world, targets_by_key[key])

        q_init = q_sol

    # --- reports -----------------------------------------------------------
    section("1. Position error (MuJoCo achieved vs SDK target, mm)")
    rows = [fmt_stats_row(key, summary(result.pos_err_mm[key]), result.pos_err_mm[key].size, "mm") for key in EEF_KEYS]
    print_table(STATS_HEADERS, rows)

    section("2. Orientation error (MuJoCo achieved vs SDK target, deg)")
    rows = [fmt_stats_row(key, summary(result.rot_err_deg[key]), result.rot_err_deg[key].size, "deg") for key in EEF_KEYS]
    print_table(STATS_HEADERS, rows)

    section("3. Solver success rate")
    pos_ok = np.stack([result.pos_err_mm[k] < args.pass_pos_mm for k in EEF_KEYS], axis=0).all(axis=0)
    rot_ok = np.stack([result.rot_err_deg[k] < args.pass_rot_deg for k in EEF_KEYS], axis=0).all(axis=0)
    pose_ok = pos_ok & rot_ok
    solver_flag = float(result.converged.mean())
    rows = [
        ["two-step solver converged (step1 & step2)", f"{int(result.converged.sum())}/{n}", f"{solver_flag * 100:.2f}%"],
        [f"MuJoCo pos < {args.pass_pos_mm} mm (torso+arms)", f"{int(pos_ok.sum())}/{n}", f"{pos_ok.mean() * 100:.2f}%"],
        [f"MuJoCo rot < {args.pass_rot_deg} deg (torso+arms)", f"{int(rot_ok.sum())}/{n}", f"{rot_ok.mean() * 100:.2f}%"],
        ["MuJoCo pose ok (all 3 frames)", f"{int(pose_ok.sum())}/{n}", f"{pose_ok.mean() * 100:.2f}%"],
    ]
